...
| Workflow n8n `Prospecção - Disparo (SMTP)` (id `nRq9R176vuXJ0aw0`) é o envio geral, não só de prospecção | 14/09/26 | Existia pronto e inativo desde 23/07: webhook `POST /webhook/prospeccao-disparo`, corpo `{to, subject, html, from?}`, remetente default `relacionamento@paaps.com.br`. É o mecanismo certo para QUALQUER e-mail avulso pelo Titan (resposta a parceiro, e-mail institucional fora do fluxo de prospecção), não só disparo de carta fria. Ativado. **Resolvido em 14/09/26 às 19h**: a Mallu rodou `automacoes/prospeccao-email/n8n/LIGAR ENVIO GERAL DE EMAIL.command` (dois cliques, fora do sandbox do Claude Code). Credencial `PAAPS Webhook Token` criada (id `Z71bRlMyKPoFajzM`), header `X-Webhook-Token` ligado ao nó do webhook, workflow ativo. Plano B pronto pra uso real, sem pendência |
