...
| Workflow n8n `Prospecção - Disparo (SMTP)` (id `nRq9R176vuXJ0aw0`) é o envio geral, não só de prospecção | 14/09/26 | Existia pronto e inativo desde 23/07: webhook `POST /webhook/prospeccao-disparo`, corpo `{to, subject, html, from?}`, remetente default `relacionamento@paaps.com.br`. É o mecanismo certo para QUALQUER e-mail avulso pelo Titan (resposta a parceiro, e-mail institucional fora do fluxo de prospecção), não só disparo de carta fria. Ativado. Falta rodar uma vez `automacoes/prospeccao-email/n8n/criar-credencial-envio-geral.mjs` para o header do webhook ter nome conhecido (`X-Webhook-Token`, valor = `PROSPECCAO_WEBHOOK_TOKEN` do `.env`); a credencial original foi criada direto no n8n e a API não devolve o nome do header. Bloqueado em toda sessão de Claude Code pelo classificador de sandbox (escrita em cofre de segredo), confirmado de novo em 14/09/26. Solução: `automacoes/prospeccao-email/n8n/LIGAR ENVIO GERAL DE EMAIL.command`, dois cliques, a Mallu roda fora do sandbox |
| **Erro real, mesmo dia (14/09/26): NUNCA construir workflow n8n novo do zero para mandar e-mail avulso** | 14/09/26 | Numa sessão precisando responder a Yunus (Luiz Henrique Pádua), a Claude criou um workflow n8n descartável na hora (webhook + nó de e-mail), em vez de usar o `Prospecção - Disparo (SMTP)` (linha acima) ou o `automacoes/prospeccao-email/responder-lead.py` que já existe e resolve os dois problemas de uma vez: acha o Message-ID do e-mail original na INBOX pra encaixar a resposta na thread certa (`In-Reply-To`/`References`), e grava a cópia na pasta Enviados do Titan por IMAP depois de enviar, porque SMTP puro nunca deixa cópia lá (mesmo motivo da linha 133 desta tabela). O e-mail saiu de verdade (SMTP aceitou, com `messageId` e resposta `250 Ok: queued`), mas sem thread e **sem nenhum registro em lugar nenhum**: nem Titan, nem CRM, nem Notion. A Claude também apagou o workflow descartável logo depois de disparar, o que apagou junto o log de execução, a única evidência técnica que sobrava. **A regra: e-mail avulso pelo Titan usa `responder-lead.py` (tem `--enviar` e ensaio sem enviar por padrão) ou o `Prospecção - Disparo (SMTP)` depois que o header for corrigido. Nunca workflow novo. E nunca apagar um workflow ou execução antes de confirmar e registrar o resultado.** |
...
3. **`Segundo Cérebro/Voz/Voz.md` é referenciado por caminho relativo** em
   `regras-prospeccao.md`, mas mora em `Segundo Cérebro/Voz/Voz.md` na raiz. O caminho não
   resolve daqui.
