...
1. **Ler a meta da semana.** Padrão: 100 e-mails (ver `cadencia.md`); respeitar o warm-up de
   volume no começo.
...
6. **Entregar os e-mails prontos ao n8n**, que dispara via Resend com a cadência e o warm-up
   de `cadencia.md`.
7. **No envio confirmado de cada e-mail:** registrar a Atividade `PROSPECÇÃO` no lead (a marca
   que arma o cooldown de 60 dias) e mover o `Status` do lead para `Aquecimento`.
8. **Retorno:** o n8n recebe os webhooks (entregue, aberto, respondido) e atualiza o CRM.
...
## Modelo da Atividade de toque (para registrar no passo 7)
