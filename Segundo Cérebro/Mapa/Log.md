...
tempo em peça pública. Registrado em [[mallu-vasconcellos]] e [[mallu-formacao-e-crp]], sem escolher
...
## [2026-09-12] reforma | o cofre virou Segundo Cérebro, em 9 setores com cor

A pasta `Conhecimento/` foi **renomeada** (nunca copiada) para `Segundo Cérebro/`, e os
seis setores numerados que eu tinha inventado sozinho deram lugar aos setores ditados
pela Mallu. As 102 notas foram movidas com `git mv`, com o conteúdo intacto.

O que a reforma corrigiu, na ordem em que ela apontou:

- **`Voz/` voltou a existir.** Tom de voz tinha sumido da estrutura. As 10 proibições e as
  7 calibrações moram lá, não em `Conceitos/`. "Sem travessão" é proibição de tom de voz,
  nunca conceito: foi esse erro que reorganizou a estrutura inteira.
- **`Conceitos/` virou o que ela disse que é:** as definições da PAAPS, que saem do TCC.
- **`Histórias/` nasceu**, e preenche o "Diretório de histórias do especialista" que está
  vazio no GTO desde 13/08/2026. Nasceu incompleto e dizendo que está incompleto.
- **Os 12 projetos dela ganharam pasta**, com os nomes dela, sem invenção.
- **`Mapa/O que falta.md` nasceu** com as 10 coisas que faltam e as 5 decisões que são
  dela, para a gente voltar nelas.
- O `Sem título.base`, arquivo vazio que era a bolinha sem nome no grafo, foi apagado.
- A pasta `Arquivos/` da raiz virou `Acervo/`, para nunca mais existirem duas pastas
  com o mesmo nome.
