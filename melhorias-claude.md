...
**Evidência:** `wiki/log/sessao-2026-07-06.md` criado pelo hook às 13:49 de hoje e nunca
...
Log de sessão passa a excluir a própria pasta `wiki/log/` do status.
...
- `workflow-paaps.html` solto na raiz. ✅ movido para `wiki/arquitetura-agentes/`.
...
**Ação:** runbook `wiki/institucional/runbook-navegacao-mcp.md` com o
...
Você usa o log de `wiki/log/` como memória entre sessões ("leia o log da última sessão" —
...
relevantes, registrar decisão/estado em `wiki/log/` (1–3 linhas) e na memória persistente
