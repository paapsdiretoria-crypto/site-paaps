...
| `fase2-aquecimento` | **Fase 2 da aquisição: alguém respondeu ao e-mail frio.** Da resposta que chegou até a Conversa de Diagnóstico agendada: ler quem respondeu, rota por tipo de resposta, escrita curta que marca reunião, horários na mesa e convite do Google Agenda, porta de entrada pelo Diagnóstico NR-1 por dispensa, registro no CRM. Gate da Mallu em toda resposta |
| `pede-segredo-a-mallu` | Um fluxo precisa de senha, token ou chave de API que só ela tem. Monta um `.command` de dois cliques com janela do macOS, grava no `.env`, testa de verdade e avisa o resultado na tela. Eu nunca digito nem vejo o valor, e ela nunca edita arquivo de configuração na mão |
